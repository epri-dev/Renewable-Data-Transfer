...
# =============================================================================
# Start:-
def start(tag_list_path, log_file_path, data_file_max_length, interval, output_dir,secret_path,channel_list_version_flag,log_sftp_path,SSH_KEY_PATH,tag_mapping_path):
...
def log_data_from_pi_new(tags_df, start_time, end_time, plant, log_excel_path, server_name, output_dir, interval,
    ...
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        os.remove(output_csv_path)
    sftp_success=upload_via_sftp(zip_file_path,secret_path,log_sftp_path,SSH_KEY_PATH)
    print(f"Data Pulled for '{plant}', {pd.to_datetime(start_time).date()}, Log files updated.")
    if sftp_success==True:
        os.remove(zip_file_path)
    for tag in tags:
        last_upload_time = log_data['timestamp'].max()
