...
from Functions.PI.pi_utils import PIPointList
from Functions.PI.channel_list_converter import convert_best_list
import logging
...
def catch_up_new_tags(plant_name, new_tags, existing_tags, plant_start_date, last_upload_time, log_excel_path, data_file_max_length, interval, output_dir, server_name,secret_path,log_sftp_path,SSH_KEY_PATH,pbar,tag_mapping_path,SFTP_en):
    ...
    while start_time < end_time:
        ...
        log_data_from_pi_new(pd.DataFrame(new_tags).transpose(), start_time, current_end_time, plant_name, log_excel_path, server_name, output_dir, interval,secret_path,log_sftp_path,SSH_KEY_PATH,tag_mapping_path,SFTP_en)
...
# =============================================================================
# Start:-
def start_best(tag_list_path, log_file_path, data_file_max_length, interval, output_dir,secret_path,channel_list_version_flag,log_sftp_path,SSH_KEY_PATH,tag_mapping_path,SFTP_en):
    df = pd.read_excel(tag_list_path, sheet_name='BEST_Tags')
    if channel_list_version_flag==1:
        df=convert_best_list(tag_list_path) #Convert the channel list to the required format
    ...
    for plant, group in (pbar := tqdm(df.groupby('PlantName'), total=tag_days)):
        ...
        while current_start_date < end_date:
            ...
            for file_no, chunk in enumerate(tag_chunks):
                ...
                log_data_from_pi_new(pd.DataFrame(chunk).transpose(), current_start_date, current_end_date, plant, plant_log_file_path, server_name, output_dir, interval,secret_path,log_sftp_path,SSH_KEY_PATH,tag_mapping_path,file_no+1,SFTP_en)
...
def log_data_from_pi_new(tags_df, start_time, end_time, plant, log_excel_path, server_name, output_dir, interval,
                     secret_path, log_sftp_path, SSH_KEY_PATH,tag_mapping_path,file_no,SFTP_en):
    ...
    try:
        ...
        sec_wait = 1800
        print(f"Stopping for {sec_wait} seconds to retry, possible PI issue...")
    ...
    if SFTP_en:
        ...
        if sftp_success:
            logging.info(f"Temporary zip file deleted: {output_csv_path}")
