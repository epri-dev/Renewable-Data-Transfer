...
def log_data_from_pi(tags, operating_state_tag, plant_level_tag, start_time, end_time,
    ...
    for tag in normal_tags:
        ...
        # )
        print(f"Simulating data retrieval for tag: {tag} from {start_time} to {end_time} with interval {interval} minutes.")
        summaries = pd.DataFrame(0, index=pd.date_range(start=start_time, end=end_time, freq=f'{interval}T'), columns=['AVERAGE', 'MINIMUM', 'MAXIMUM', 'STD_DEV'],index_name='timestamp')
        ...
        print(f"the summaries data frame has these columns: {df.columns.tolist()} and these many rows: {len(df)}")
    ...
    if op_tag:
        ...
        #     point = all_points[op_tag]
            ...
            #     time_type=TimestampCalculation.EARLIEST_TIME
            # )
        print(f"Simulating data retrieval for operating state tag: {op_tag} from {start_time} to {end_time} with interval {interval} minutes.")
        summaries = pd.DataFrame(0, index=pd.date_range(start=start_time, end=end_time, freq=f'{interval}T'), columns=['AVERAGE'],index_name='timestamp')
        df = summaries.reset_index()
        df.rename(columns={'AVERAGE': f'{op_tag}_Avg'}, inplace=True)
        df.rename(columns=tag_mapping_dict, inplace=True)
        print(f"the summaries data frame has these columns: {df.columns.tolist()} and these many rows: {len(df)}")
        # else:
        #     df = None
    ...
    if turbine == "PlantLevel" and plant_level_tag:
        ...
        #         time_type=TimestampCalculation.EARLIEST_TIME
        #     )
        print(f"Simulating data retrieval for plant level tag: {plant_level_tag} from {start_time} to {end_time} with interval {interval} minutes."
              )
        summaries = summaries = pd.DataFrame(0,index=pd.Index(pd.date_range(start=start_time, end=end_time, freq=f'{interval}T'), name="Timestamp"),columns=['AVERAGE', 'MINIMUM', 'MAXIMUM', 'STD_DEV']
)
        ...
        df.rename(columns=tag_mapping_dict, inplace=True)
        print(f"the summaries data frame has these columns: {df.columns.tolist()} and these many rows: {len(df)}")
        # plant_level_results.index.name = 'timestamp'
