# =============================================================================
# EPRI Developed Data Export Adapter Packet: BEST Reliability Data Transfer
...
from Functions.Canary.channel_list_converter import convert_channel_list
...
# =============================================================================
# log_tag_details:-
def log_tag_details(plant_name, tag_names, upload_times, log_excel_path):
    try:
    ...
    upload_time = pd.to_datetime(upload_times['timestamp'].max()).replace(microsecond=0).strftime('%Y-%m-%d %H:%M:%S')
...
def catch_up_new_tags(plant_name, new_tags, existing_tags, plant_start_date, last_upload_time, log_excel_path, data_file_max_length, interval, output_dir, server_name,secret_path,log_sftp_path,SSH_KEY_PATH,pbar,tag_mapping_path,SFTP_en):
    ...
    while start_time < end_time:
        ...
        log_data_from_canary(pd.DataFrame(new_tags).transpose(), start_time, current_end_time, plant_name, log_excel_path, output_dir, interval,secret_path,log_sftp_path,SSH_KEY_PATH,tag_mapping_path,SFTP_en=SFTP_en)
...
# =============================================================================
# Start:-
def start_best(tag_list_path, log_file_path, data_file_max_length, interval, output_dir,secret_path,channel_list_version_flag,log_sftp_path,SSH_KEY_PATH,tag_mapping_path,SFTP_en=True):
    df = pd.read_excel(tag_list_path, sheet_name='BEST_Tags')
    if channel_list_version_flag==1:
        df=convert_channel_list(tag_list_path) #Convert the channel list to the required format
    ...
    for plant, group in (pbar := tqdm(df.groupby('PlantName'), total=tag_days)):
        ...
        plant_log_file_path = os.path.join(log_file_path, f"{plant}BEST_log.csv")
        ...
        tags = []
        for col in group.columns[3:-1]: # column 3 (fourth column) is mapped to Column D in Excel
        ...
        # if new_tags:
        #     tags = catch_up_new_tags(plant, new_tags, existing_tags, plant_start_date, last_upload_time, plant_log_file_path, data_file_max_length, interval, output_dir, server_name,secret_path,log_sftp_path,SSH_KEY_PATH,pbar,tag_mapping_path,SFTP_en=SFTP_en)
        
        tags_per_files = 500
        tag_chunks = [tags[i:i + tags_per_files] for i in range(0, len(tags), tags_per_files)] # set maximum amount of tags per call, due to DSP best practice
        interval_delta = timedelta(minutes=interval) 
        ...
        while current_start_date < end_date:
            ...
            for file_no, chunk in enumerate(tag_chunks):
                print(f'Pulling data {pd.to_datetime(current_start_date).date()} - {pd.to_datetime(current_end_date).date()}, group {file_no+1:02d}')
                log_data_from_canary(pd.DataFrame(chunk).transpose(), current_start_date, current_end_date, plant, plant_log_file_path, output_dir, interval,secret_path,log_sftp_path,SSH_KEY_PATH,tag_mapping_path, file_no+1,SFTP_en=SFTP_en)
            current_start_date = current_end_date + interval_delta
            

# =============================================================================
# log_data_from_canary:-
def log_data_from_canary(tags_df, start_time, end_time, plant, log_excel_path, output_dir, interval,
                     secret_path, log_sftp_path, SSH_KEY_PATH, tag_mapping_path, file_no, SFTP_en=True):
    ...
    # Fetch data from Canary API
    try:
        log_data = conn.get_aggregate_data(tags, start_str, end_str, f'{interval}m', 'TimeAverage2')
    except Exception as e:
        sec_wait = 1800
        print(f"Error fetching data from Canary API: {e}")
        print(f"Stopping for {sec_wait} seconds to retry...")
        time.sleep(sec_wait)  # Wait for sec_wait seconds before retrying
        log_data = conn.get_aggregate_data(tags, start_str, end_str, f'{interval}m', 'TimeAverage2')
    ...
    # output_csv_path = os.path.join(output_dir, f'{plant.replace(' ', '_')}BEST_File{file_no:02d}_{current_time}.csv')
    filename = f'{plant}BEST-File{file_no:02d}_{start_time.strftime('%Y%m%d')}.csv'
